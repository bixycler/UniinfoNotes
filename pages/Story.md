			...
			- 20th,...
			  collapsed:: true
				- Newtonian mechanics → [Lagrangian mechanics](https://en.wikipedia.org/wiki/Lagrangian_mechanics) → field equations
					...
					- For each particle: $\sum {\bf F}_i = {\bf F}_i^{(a)} + {\bf F}_i^{(n)} = {\bf F}_i^{(cs)} + {\bf F}_i^{(nc)} + {\bf F}_i^{(n)}$
						- (a) = applied force, (n) = normal constraint force, (cs) = conservative force, (nc) = nonconservative force
						-
