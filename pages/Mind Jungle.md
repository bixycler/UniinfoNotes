					  ...
					  id:: 699c0360-af75-41a2-a58a-f7875f453ff9
						...
						- [Google AI Studio](https://aistudio.google.com/)
						  collapsed:: true
							...
							- ⚠️ Warning: _**Cost information may take up to 24 hours to update.**_ (in [Spend](https://aistudio.google.com/spend) & [Billing](https://aistudio.google.com/billing))
			  ...
			  The coding agent open-sourcing Claude Code
				...
				- Settings
					...
					- For JSON key of Google Vertex AI
					  ...
					  collapsed:: true
						- [!] Cannot use the UI “Connect provider”, because `Vertex` provider there only supports API key, not JSON key file.
						- ⇒ Manually add `google-vertex` with `googleAuthOptions`:`keyFilename` to `opencode.json`.
							- `opencode.json`
							  ```json
							  "google-vertex": {
							    "options": {
						...
						- For Vertex AI via Cloudflare: encode all options into the `baseURL`
							...
							- So, we must create a new provider `cloudflare-vertex-ai`.
								- Note: While both `cloudflare-vertex-ai` & `cloudflare-ai-gateway` don't support reasoning (just like `openrouter`), DeepSeek models can think with no problem, but Gemini models cannot think.
							...
							- Complete URL: `https://gateway.ai.cloudflare.com/v1/$CF_ACCOUNT_ID/$CF_GATEWAY_ID/google-vertex-ai/v1/projects/$GOOGLE_CLOUD_PROJECT/locations/$VERTEX_LOCATION/publishers/google/models/gemini-3.1-flash-lite:generateContent`
							- `opencode.json`
							  ```json
							  "cloudflare-vertex-ai": {
							    "name": "Cloudflare Vertex AI",
							    "npm": "@ai-sdk/google-vertex",
							    "options": {
							      "baseURL": "https://gateway.ai.cloudflare.com/v1/0aeb7365d576088492b5b2760cc47b12/ai-gateway/google-vertex-ai/v1/projects/vertex-ai-agent-499213/locations/global/publishers/google"
							    },
							    "models": {
					...
					- The state of the art of the codebase knowledge graph engines
					  ...
					  collapsed:: true
						...
						- Dynamic Behavior Limitations: Static parsers cannot track runtime dynamic behaviors, such as reflection (in Java, etc.), dynamic imports (for interpreted languages like Python and JavaScript), or runtime dependency injection.
							...
							- Agentic Runtime Debugging and Dynamic Call Graphs: [Agent-centric Debugging Interface (ADI)](https://arxiv.org/html/2604.24212v1) is a specialized, function-level interaction paradigm designed to make dynamic analysis highly cost-efficient for autonomous AI agents. Unlike traditional software debugging that relies on line-by-line execution, ADI provides AI agents with high-level structural overviews and function-level controls. The core components include: Frame Lifetime Trace (FLT), Dynamic Call Graphs, Conditional Breakpoints
		...
		- refinement techniques
		  ...
		  collapsed:: true
				  ...
				  “Hít vào chấp nhận, thở ra buông xả”
					...
					- Mantras
						- Let it be, let it go, let it flow!
							- ((6651ecba-793d-43c5-8020-a9f260b032d8)) Accept the form as is, release the grasp, and let the thread flow.
							...
							- Orders & flows
							  collapsed:: true
								...
								- ((6651adea-81d4-40a5-b96d-af6739e572eb)):
									- ((67139355-ac72-4e4c-b882-00bb3a3ea144)): _obsoleted by the [Logseq's built-in automatic Git commits](((699c035f-9972-4df3-9bc1-71b992426a93)))_
						...
						- Complete failure!
							...
							- Reason: Planning (life programming) is ineffective to me due to my non-verbal thinking nature.
							  id:: 699e6a05-0095-46dd-a179-0e4c634a41c2
								- In my deep flow, there's no notion of time, only the present crushed by the heavy [stack of intents](((690b3f49-1419-4dc3-b61b-12890b9e04f9))). No program can be stored in the stack frames, only the intents created by my ((681b3ad7-d1b0-4c06-a4ea-385a4a9314e0)).
			  ...
			  ((665359e4-4597-4775-b849-f9acbb98960a)) ((68898917-e61f-4d23-9124-474efa9a9eee))
				- look behind the eyes
				  ...
				  ((665c9af1-1ce2-461c-af33-671690618c8f)) ((6889c11f-e881-432f-ba0f-876b61afc60c))
					...
					- ((665359ff-79f1-4669-b10b-f2b0e633a7c1))
						- Because ((66b1cfa4-01ef-4ee8-9409-32c9884c39cd)) is what's behind the eyes, it is normally an invisible, hidden boss.
			...
			- ### being present
			  ...
			  :END:
				- ((6651ecba-793d-43c5-8020-a9f260b032d8)) Throughout life, we're mostly enslaved to the past ((665ca429-84e3-49ff-921e-c07d19cd99ba))s – our ((66b1cfa4-01ef-4ee8-9409-32c9884c39cd))s – which constantly command us to replicate them in our images about the future. To cut this chain of slavery, we need to practice meditation to [absorb into the present](((6850c7e4-0969-45b7-9ca8-e2c1d764effe))).
				- ((665359ff-79f1-4669-b10b-f2b0e633a7c1))
							...
							- ((665359ff-79f1-4669-b10b-f2b0e633a7c1))
								- Living with the perfect imperfection: *Don't force the force!*
								  ...
								  collapsed:: true
									...
									- The “meta-force” is to the objective force just as the second arrow is to the first arrow of [suffering](((69f99afb-5535-4929-8d01-fbc804b91170))) – the disfunctional vs the functional.
									- This stance is called [Wu Wei (無為) in Taoism](https://en.wikipedia.org/wiki/Wu_wei), the way Jedi masters relate to [the Force](https://en.wikipedia.org/wiki/The_Force) in *[Star Wars](https://en.wikipedia.org/wiki/Star_Wars)* fiction franchise, and also the way master divers deal with the great water.
							...
							- DONE [!] All block IDs are cleared after search with advanced query.
							  ...
							  :END:
								...
								- Today, after a search for `water scoop`... the matches in [[Uniinfo]] somehow cleared all block IDs there 😮🤔, which was auto-saved in commit `449c9135`!
									...
									- DONE Now, i close Logseq, remove the graph file to force a re-index.
									  :LOGBOOK:
				...
				- `Settings` > `Version control`
				  id:: 66fa4733-114e-4923-80a5-7ac2f5f63ae1
					- `Enable Git auto commit` is useful for **periodic backup**.
					  id:: 699c035f-9972-4df3-9bc1-71b992426a93
