					  ...
					  collapsed:: true
						...
						- For Vertex AI via Cloudflare: encode all options into the `baseURL`
							...
							- So, we must create a new provider `cloudflare-vertex-ai`.
								- Note: While both `cloudflare-vertex-ai` & `cloudflare-ai-gateway` don't support reasoning, DeepSeek models can think with no problem, but Gemini models cannot think.
