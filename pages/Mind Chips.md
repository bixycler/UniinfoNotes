	...
	- DOING Add `thoughtSignature` to history
	  ...
	  :END:
		...
		- Define `<thought>...</thought>` format in system prompt
			- This makes Gemini 2.5 *think out loud* with `<thought>...</thought>` instead of `thought: true` parts!
			- Gemini 3 still keeps `thought: true` parts, not outputting `<thought>...</thought>`.
