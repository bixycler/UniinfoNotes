## Git <a class="logseq-meta" id="666ba1e2-19d1-409e-b30e-42a99b7e4ec0" ></a>
	...
	- [Commentaries:](../../../../publish/CommonMark/Namespace.cm.md#665359ff-79f1-4669-b10b-f2b0e633a7c1) <a class="logseq-meta" data-collapsed="true" ></a>
		...
		- `\ No newline at end of file` <a class="logseq-meta" id="556c799a-e364-49ad-94ab-5d14bef766ca" data-collapsed="true" data-logbook="CLOCK: [2025-01-21 Tue 14:07:59]--[2025-01-24 Fri 19:00:42] =&gt;  76:52:43&#10;" ></a>
		  <br>is the warning of `diff` because `diff` is a **line-based** text processor.
			...
			- 1st, `diff` works with the whole line, hence, the last line will be marked as modified by the next commit.
				- E.g. wrong lines in diffs between commits without the last newline: <a class="logseq-meta" data-collapsed="true" ></a>
					- Final file
					  ```
					  Test EOL@EOF⏎
			...
			- 2nd, the last line of the last commit may be [blamed](https://git-scm.com/docs/git-blame) for the next commit, which is misleading.
				- Some git implementations do this misleading blame: GitHub, GitLab, VS Code <a class="logseq-meta" data-collapsed="true" ></a>
			...
			- Commands to refactor codes <a class="logseq-meta" id="b8c17a55-f618-43ed-9826-314412a08965" data-collapsed="true" ></a>
				...
				- Find files with the last newline
				  ...
				  ``` <a class="logseq-meta" id="577c2916-d4ca-461d-85b9-f3b2ff7e6b30" ></a>
	- working tree <a class="logseq-meta" id="67152d29-5cee-475d-a01b-bbc9c9ad3417" data-collapsed="true" ></a>
		...
		- multiple working trees per repo <a class="logseq-meta" id="67163453-4d1b-492d-ab06-532cb37e7431" ></a>
		  <br>`git worktree`
			- Official docs: [Manage multiple working trees](https://git-scm.com/docs/git-worktree)
			- [Description:](../../../../publish/CommonMark/Namespace.cm.md#6651ecba-793d-43c5-8020-a9f260b032d8) We can configure many separate views as separate views of the same repo, e.g. to checkout and manage different branches with no need to clone the same remote repo into many local repos.
			- Commands
				- ```sh
				  ...
				  ``` <a class="logseq-meta" id="68500a6e-0c83-4650-8bcd-9f1602b80ee9" ></a>
			- Structure: one main working tree containing [`$GIT_DIR`](#67152861-f595-4ad1-88a9-9363082d03eb), and many linked working trees containing [`gitfile`](#67152b95-02b4-473b-a88b-6cbab4b46749) pointing to `${GIT_DIR}/worktrees/${view}`.
				- worktree
				  <br>= working tree + repository metadata ( [`$GIT_DIR`](#67152861-f595-4ad1-88a9-9363082d03eb))
	...
	- `$GIT_DIR` <a class="logseq-meta" id="67152861-f595-4ad1-88a9-9363082d03eb" data-collapsed="true" ></a>
	  <br>`--git-dir`
		- Official docs: [Git Repository Layout](https://git-scm.com/docs/gitrepository-layout)
		- [Description:](../../../../publish/CommonMark/Namespace.cm.md#6651ecba-793d-43c5-8020-a9f260b032d8) [`$GIT_DIR`](#67152861-f595-4ad1-88a9-9363082d03eb) contains the whole Git repo whose current state is the [working tree](#67152d29-5cee-475d-a01b-bbc9c9ad3417). It's default to a folder named `.git` for repo with working tree, or `${repo}.git` for bare repo without working tree, which is by default at the root of the working tree.
			- Non-default `$GIT_DIR` can be specified by env.var. `${GIT_DIR}` or `git --git-dir` argument.
			- [`$GIT_DIR`](#67152861-f595-4ad1-88a9-9363082d03eb) can also be a specified indirectly through a [`gitfile`](#67152b95-02b4-473b-a88b-6cbab4b46749). The assignment `GIT_DIR=$gitfile` or `--git-dir=$gitfile` is an abuse of terminology!
	...
	- nested repository <a class="logseq-meta" data-collapsed="true" ></a>
		- When adding folder `$subrepo` containing `.git` to another (outer) git repo, its contents cannot be added. Only one *file* `$subrepo` is added as an anchor to the current `HEAD` commit of a nested repo. <a class="logseq-meta" data-collapsed="true" ></a>
			- warning: `adding embedded git repository: $subrepo`
			  ...
			  ```
		- submodule <a class="logseq-meta" id="67151eb0-94a3-47bb-a7f9-25561690e75d" ></a>
			...
			- Note: A submodule usually use [`gitfile`](#67152b95-02b4-473b-a88b-6cbab4b46749) instead of the normal `.git/` folder, so that the outer repo can `git checkout` a branch that does not have this submodule. That's because the `checkout` has to remove the entire submodule's working tree, without losing the submodule's repo info.
		- nested repo as a normal folder
			...
			- If `$subrepo` has been accidentally added as a nested repo, it must be removed first
			  ...
			  ```
			- Optionally, `$subrepo/.git` can also be `.gitignore`d by the outer repo. But this is unnecessary: By default, Git ignores all `.git` files/folders, because that's the default name for [`$GIT_DIR`](#67152861-f595-4ad1-88a9-9363082d03eb).
			- References:
	...
	- Useful commands (sets) <a class="logseq-meta" id="666022fc-2700-438d-810e-a6fab07f696f" data-collapsed="true" ></a>
		- List files changed:
		  <br>`git {diff,show,log} --{name-{only,status},stat}`
		...
		- Force pull
			- To **overwrite** this branch with its upstream,
			  ...
			  ``` <a class="logseq-meta" id="67af0c5c-996c-4065-9533-0ce9fce905bf" ></a>
			...
			- To **merge** with upstream even if the remote tracking branch `@{u}` is diverged from the branch on remote repo,
			  ...
			  ``` <a class="logseq-meta" data-collapsed="true" ></a>
				- which is equivalent to (depending on `git config pull`)
				  ...
				  ```
		- Merge
			- Merge favoring `ours`/`theirs` over the other side (using default strategy `-s ort`) <a class="logseq-meta" id="666172e6-15fa-412e-a2ba-a32304da6937" ></a>
			  <br>`git merge -X theirs`, `git merge --strategy-option=theirs`
			- Merge with log & merge commit (without fast-forward):    
			  <br>`git merge --no-ff --log --signoff`  
			  <br>Use this to force a merge commit even if it can be fast-forwarded, and collect all titles of the merged commits into this merge commit message.
			- `merge --squash` = `cherry-pick -m1` to squash a merge into a single commit for linear history. <a class="logseq-meta" data-collapsed="true" ></a>
				...
				- `feature:F> git merge project > m2`: commit `m2` has 2 parents, 1st `F` and 2nd `E`. <a class="logseq-meta" data-collapsed="true" ></a>
					- History of `feature` does contain the 2 merged commits `D` & `E`, and
					- The changes of `m2` is relative to each parent:
						- relative to 1st parent `F`, changes of `m2` = changes of `D` & `E` <a class="logseq-meta" id="669f711c-67b5-4e6e-94c8-ac64e2bd861b" ></a>
			...
			- Merge specific files (interactive checkout)
			  <br>`git checkout --patch ${branch} [--] ${file1}, ${file2}, ...`
				- [List of commands, as shown in `INTERACTIVE MODE` > `patch` of `git help add`](#6758f04c-5604-475f-bf81-d9739200e4e6)
		- Selectively add diff hunks (patches):   
		  <br>`git add --patch`  
		  <br>Use this to interactively search for appropriate diff hunks via `/`, then stage them with `y` or skip with `n`, or even manually edit hunks with `e`, and so on.
			- List of commands, as shown in `INTERACTIVE MODE` > `patch` of `git help add` <a class="logseq-meta" id="6758f04c-5604-475f-bf81-d9739200e4e6" data-collapsed="true" ></a>
		...
		- Range of commits (revision range)
			- `${base_exclusive}..${head_inclusive}` means “all commits reachable from `${head_inclusive}` that aren't reachable from `${base_exclusive}`”. <a class="logseq-meta" data-collapsed="true" ></a>
				- `^${ref}` means "excluding all reachable commits from `${ref}`", hence
				  <br>`${base_exclusive}..${head_inclusive}` means `^${base_exclusive} ${head_inclusive}`
			- `${base_inclusive}~..${head_inclusive}` means roughly "from `${base_exclusive}` to `${head_inclusive}`", and exactly “all commits reachable from `${head_inclusive}` that aren't reachable from  the first parent of `${base_exclusive}`”. <a class="logseq-meta" data-collapsed="true" ></a>
			...
			- Ref: [Git docs](https://git-scm.com/book/en/v2/Git-Tools-Revision-Selection#_commit_ranges) <a class="logseq-meta" id="66b1cfa5-6161-4074-bd22-077ef848026b" ></a>
		- Find the common base of branches
			- Between this and upstream
			  ```sh
			  git merge-base HEAD @{u}
			  ```
	- Git workflows <a class="logseq-meta" id="666022fc-eeeb-4365-a854-7e14045655be" data-collapsed="true" ></a>
		- [Git:](../../../../publish/CommonMark/Namespace.cm.md#665f1a5c-6c98-4785-a177-3cd01507595d) https://github.com/bixycler/GitWorkflows
		- `work`-`feature`-`project` workflow <a class="logseq-meta" id="6686731f-e794-4d7a-8e56-09128b3224b2" data-collapsed="true" ></a>
			- [Description:](../../../../publish/CommonMark/Namespace.cm.md#6651ecba-793d-43c5-8020-a9f260b032d8) The `work` branch is the working place storing all fine-grained commits, including 3 types of commits: project commits (in cyan) from `project` branch,  feature commits (in blue) to be cherry-picked to `feature` branch, and local commits (in red) for local development environment only. The interactions between these branches are as following:
			  <br>`project` >-[merge --squash]-> `work`  >-[cherry-pick]->  `feature`  <-[rebase/merge]->  `project`
			...
			- Branches
				- `work` branch
				  <br>is the working branch keeping the full-detail history of all works.
					...
					- Use `merge --squash` to get codes from `project`
						- ```sh
						  ...
						  ```
				- `feature` branch
				  <br>is the middleman to bridge between the linear `work` branch and the octopus `project` branch.
					...
					- `cherry-pick` feature commits from `work` for merge-request to `project`
						- ```sh
						  ...
						  ``` <a class="logseq-meta" id="66868431-a1d0-4df9-a250-0f8bb7bc1a56" ></a>
					- `rebase project` (and optionally squash feature commits them down with option `-i`) before creating merge request to `project` branch
						- ```sh
						  ...
						  ``` <a class="logseq-meta" id="669f79cc-08d5-4507-bd72-f8e1af4892d8" ></a>
						- Note: on the Merge request page, there's an option "Squash commits", but it's better to squash before hand with `rebase -i`.
				- `store` branch
				  <br>is an optional branch to store coarse-grained commits of `work` branch corresponding to merge requests to `project`.
					- Whenever a [merge request](#669f79cc-08d5-4507-bd72-f8e1af4892d8) is created, the corresponding chunk of `work` commits should be stored to `store` branch via a `merge --no-ff --log`.
						...
						- This command can be stored as alias in `~/.gitconfig`.
							- ```ini
							  ...
							  ```
		- Revise commits from downstream (local, personal) to upstream (remote, redmine, project) <a class="logseq-meta" data-collapsed="true" ></a>
			...
			- In case of many commits to upstream: (for project)
				- 
				  ...
				  #...
				  ``` <a class="logseq-meta" id="66b1cfa5-1ec8-4313-b635-2b22d705d19f" ></a>
			- Notes: Use `git checkout stash -- .` instead of `git stash apply` to overwrite current workdir <a class="logseq-meta" data-collapsed="true" ></a>
				- Ref: https://stackoverflow.com/a/16625128/789095
	- Troubleshooting <a class="logseq-meta" id="66fd1d03-f0d8-41b6-ba5e-7f040d343e33" data-collapsed="true" ></a>
		- Env.var.
		  ```sh
		...
		- Config.var.
			...
			- command option `-c`, `--config-env`
			  ...
			  ```
		- `fsck` to check dangling commits, dangling objects, when the repo is corrupt. <a class="logseq-meta" id="6808b9c5-402d-4923-997b-7b8056f71dc8" data-logbook="CLOCK: [2025-04-23 Wed 16:58:38]&#10;" ></a>
		...
		- References
			...
			- Git config - [Variables](https://git-scm.com/docs/git-config#_variables) <a class="logseq-meta" id="66fa6b26-2c9a-43d6-9856-18321672d800" ></a>
	...
	- Known issues <a class="logseq-meta" data-collapsed="true" ></a>
		- ⚠️ GitLab automatic merge conflict resolution just creates a **reverse merge** commit in the source branch before merging to the target brance.
		  <br>⇒ Don't let GitLab resolve conflict!
