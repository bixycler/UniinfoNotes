	  ...
	  collapsed:: true
		...
		- DOING `\ No newline at end of file`
		  ...
		  is the warning of `diff` because `diff` is a **line-based** text processor.
			...
			- 3rd, pseudo conflict: When merging many branches modifying the same file missing last newline, only one branch appending to the file is enough to make **all other branches conflict**, even when they modify completely different lines without overlapping.
