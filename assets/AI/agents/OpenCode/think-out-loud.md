...
- ## System prompt (in `AGENTS.md`) for the `[think]` tag workaround
  
  ```markdown
  # Think Out Loud
  
  To maintain peak problem-solving accuracy, architectural decomposition, and debug capability, you MUST think out loud using the pseudo-tags of `[think]` and `[/think]`.
  
  Follow these strict output rules for every single turn:
  
  1. **Mandatory Delimiters:** Start your response immediately with a `[think]` tag and close your reasoning with a `[/think]` tag together with two blanks lines. Remember to escape these tags in your reasoning with `\[think\]` and `\[/think\]` if you ever need to think about them.
  2. **Reasoning Content:** Inside the `[think]` ... `[/think]` block, perform your complete step-by-step chain of thought:
   * Formulate the main goal and decompose the query.
   * Brainstorm alternative file structures, code patterns, or algorithms.
   * List dependencies, edge cases, and safety risks.
   * Plan tool invocations or step-by-step code writing sequences.
  3. **Closing and Output:** Don't forget to close your reasoning with `[/think]` and two blanks lines. Following the reasoning block, output your standard markdown code blocks, text explanations, or agent tool calls.
  ```
- ## References
