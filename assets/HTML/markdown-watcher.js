...
function restructureToFolderDiv(node){
    ...
    // Note: Use looseList to wrap all item contents into <p>, then use node.children[0], instead of node.childNodes[]
    let title = node.children[0]; // don't use , because it  (all texts are wrapped in <p>, no bare text node is generated by markdown-it)
